...
// ------------------------------------------
//   v5.3.8
//     License changed from MIT to GPL-3.0 (LICENSE, manifest, .umod, header).
//     Versions <= 5.3.7 remain MIT. No code changes.
//
//   v5.3.7
//     Adopted the DunganSoft Plugin Standard: canonical manifest/.umod schema,
//     LICENSE rename, SECURITY/.editorconfig/.gitattributes, vendored conformance
//     checker + standards CI, canonical release workflow. No code changes.
//
//   v5.3.6
...
{
    [Info("ModernNoCupboardDecay", "gjdunga", "5.3.8")]
    ...
    public class ModernNoCupboardDecay : RustPlugin
    {
        ...
        private bool IsOwnerAuthorizedOrTeammate(BuildingPrivlidge priv, ulong ownerId)
        {
            ...
            if (authList != null)
            {
                foreach (ulong authedId in authList)
                {
                    if (ownerTeam.members.Contains(authedId))
                        return true;
                }
            }
            return false;
        ...
        private static bool IsAuthorizedOnCupboard(BuildingPrivlidge priv, ulong ownerId)
        {
            ...
            foreach (ulong authedId in authList)
            {
                if (authedId == ownerId) return true;
            }
            return false;
        }
